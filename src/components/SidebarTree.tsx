...
export default function SidebarTree({
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Search input — hidden when editor is open */}
      {!showEditor && (
      ...
      {/* Content area: NavEditorPanel or nav tree */}
      {showEditor ? (
      ...
      <div className="shrink-0 border-t bg-muted/10">
        ...
        {isAdmin && (
          <button
            ...
            className={cn(
              "w-full flex items-center gap-2 px-3 py-1.5 text-[11px] transition-colors border-t",
              showEditor
                ? "bg-muted font-medium text-foreground"
                : "text-muted-foreground hover:bg-muted/50"
            )}
